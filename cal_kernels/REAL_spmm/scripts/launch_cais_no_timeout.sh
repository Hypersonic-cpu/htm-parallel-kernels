...
if [[ $# -lt 3 ]]; then
  ...
  echo "Profiles: mu4rq16 | tuneA | tuneB | tuneC | tuneD | tuneE | tuneF | tuneG | tuneH | tuneI | tuneJ | bypassA | bypassB"
  exit 1
fi
...
cd "${KERNEL_DIR}"
make ARCH=cais CONF=GH100 RUN_ROOT="${RUN_ROOT}" RUN_ARGS="${RUN_ARG}" SIM_LIBCUDART_SO="${SIM_LIBCUDART_SO:-}" run report
